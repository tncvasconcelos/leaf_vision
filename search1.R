...
for(i in 1:length(first_batch)) {
  metadata <- search_specimen_metadata(taxon_name=first_batch[i], limit=100, hasCoordinate=T)
...
second_batch <- taxized_names[101:200]
for(i in 1:length(second_batch)) {
  metadata <- search_specimen_metadata(taxon_name=second_batch[i], limit=100, hasCoordinate=T)
  # Removing duplicates based on coordinates
  metadata <- subset(metadata, !duplicated(paste0(metadata$decimalLatitude,"_",metadata$decimalLongitude)))
  if(nrow(metadata)>5) {
    # Adding preference scores
  ...
  write.csv(metadata, file=paste0("virtual_herbarium_NPleafPaper/metadata/",metadata$scientificName[1]," metadata.csv"))
  try(download_specimen_images(metadata,
                               dir_name="virtual_herbarium_NPleafPaper/search101_200",
                               result_file_name=paste0("virtual_herbarium_NPleafPaper/search101_200/",metadata$scientificName[1])))
  
  cat(i, "\r")
}

third_batch <- taxized_names[201:600]
for(i in 1:length(third_batch)) {
  metadata <- search_specimen_metadata(taxon_name=third_batch[i], limit=100, hasCoordinate=T)
  # Removing duplicates based on coordinates
  metadata <- subset(metadata, !duplicated(paste0(metadata$decimalLatitude,"_",metadata$decimalLongitude)))
  if(nrow(metadata)>5) {
    # Adding preference scores
  ...
  write.csv(metadata, file=paste0("virtual_herbarium_NPleafPaper/metadata/",metadata$scientificName[1]," metadata.csv"))
  try(download_specimen_images(metadata,
                               dir_name="virtual_herbarium_NPleafPaper/search201_600",
                               result_file_name=paste0("virtual_herbarium_NPleafPaper/search201_600/",metadata$scientificName[1])))
...
fourth_batch <- taxized_names[601:993]
for(i in 24:length(fourth_batch)) {
  metadata <- search_specimen_metadata(taxon_name=fourth_batch[i], limit=100, hasCoordinate=T)
  # Removing duplicates based on coordinates
  metadata <- subset(metadata, !duplicated(paste0(metadata$decimalLatitude,"_",metadata$decimalLongitude)))
  if(nrow(metadata)>5) {
    # Adding preference scores
  ...
  write.csv(metadata, file=paste0("virtual_herbarium_NPleafPaper/metadata/",metadata$scientificName[1]," metadata.csv"))
  try(download_specimen_images(metadata,
                               dir_name="virtual_herbarium_NPleafPaper/search601_993",
                               result_file_name=paste0("virtual_herbarium_NPleafPaper/search601_993/",metadata$scientificName[1])))
