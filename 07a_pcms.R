...
summary(avg_model)
model_summary_output <- capture.output(summary(avg_model))
writeLines(model_summary_output, "tables/model_summary_output_07a_lp.csv")

coef_summary <- summary(avg_model)$coefficients
write.csv(avg_model$msTable, file = "tables/model_fits_07a_lp.csv", row.names = TRUE)
write.csv(coef_summary, file = "tables/modelavg_coefficients_07a_lp.csv", row.names = TRUE)
