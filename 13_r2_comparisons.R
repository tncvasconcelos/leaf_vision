...
# 07a
full_model <- readRDS(file = "models/full_model_07a_lp.rds")
model_set <- readRDS(file = "models/model_set_07a_lp.rds")
phy <- read.tree("trees/phy_070a_lp.tre")
data_subset <- dat <- full_model$model.frame
full_formula <- full_model$formula
...
model_avg_lm <- model.avg(model_set_lm, subset = delta < 2)
avg_coefs <- coef(model_avg_lm)
formula_avg <- as.formula(paste("lma ~", paste(gsub("evergreen", "", names(avg_coefs))[-1], 
  collapse = " + ")))
avg_model_lm <- lm(formula_avg, data = dat)
